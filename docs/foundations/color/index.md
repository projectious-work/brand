...
[the code-comment token](/brand/docs/interface/code/) for a worked
example.
</div>
...
- **Direct labelling with one highlight.** Draw every series in `slate-7`, draw
  ...
  [deck](/brand/docs/media/presentations/) — one idea per slide holds
  for charts too.
...
[table numerics are](/brand/docs/interface/components/): digits have
...
[Dark mode](/brand/docs/interface/dark-mode/) for the implementation
