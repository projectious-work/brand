# Portfolio presentation

> Truthful status, social-preview, diagram, and demo patterns.

---

LLMS index: [llms.txt](/brand/llms.txt)

---

Portfolio presentation must make maturity and limitations easier to see, not
make prototypes appear more complete than they are.

- [Status treatments](/brand/docs/portfolio/status/) define the approved maturity vocabulary.
- [Social previews](/brand/docs/portfolio/social-previews/) define reusable repository artwork.
- [Diagrams and demos](/brand/docs/portfolio/diagrams/) define evidence and source requirements.
- [Documents](/brand/docs/portfolio/documents/) define the one-page CV, proposal, and memo pattern.

These patterns are projectious.work organisation assets. Bernhard Gerlach's
personal LinkedIn profile is a separate identity: use his name, portrait, and
first-person voice only with his approval. Do not turn a personal profile into
an organization-brand channel or imply personal endorsement.

---

Section pages:

- [Status treatments](/brand/docs/portfolio/status/): Text-first maturity labels for truthful portfolio presentation.
- [Social previews](/brand/docs/portfolio/social-previews/): Repository artwork that states category and maturity plainly.
- [Diagrams and demos](/brand/docs/portfolio/diagrams/): Evidence-first architecture, flow, screenshot, and demo framing.
- [Documents](/brand/docs/portfolio/documents/): The one-page document pattern — CV, proposal, memo — and how it paginates.
